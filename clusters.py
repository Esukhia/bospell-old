...
class BasicTests:
    ...
    def has_skrt_char(self, token):
        return self.SKRT_VOW in token.char_groups or \
               self.SKRT_CONS in token.char_groups or \
               self.SKRT_SUB_CONS in token.char_groups
...
class MatcherTests(BasicTests):
    ...
    def mono_or_punct_with_skrt_cluster(self, clusterized):
        result = []
